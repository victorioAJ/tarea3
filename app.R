...
library(DescTools)
library(shinythemes)
library(report)  # <--- Importante para la interpretación automática
    ...
    mainPanel(
      tabsetPanel(
        ...
        tabPanel("🧪 Resultados del Análisis",
                 ...
                 br(),
                 h5("🧠 Interpretación detallada usando 'report':"),
...
server <- function(input, output) {
  ...
  output$interpretacion_report <- renderPrint({
    res <- resultado_analisis()
    if (!is.null(res) && inherits(res, "aov")) {
      cat(report(res))
    } else {
      cat("Interpretación automática no disponible para este tipo de análisis.")
    }
  })
}
